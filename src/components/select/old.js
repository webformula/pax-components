customElements.define('mdw-select-old', class extends HTMLElementExtended {
  constructor() {
    super();
  ...
  setupEnhanced_() {
    ...
    if (MDWUtils.isMobile) {
      document.body.insertAdjacentHTML('beforeend', this.sheetHTML(`<div class="options-list">${this.selectElement.innerHTML}</div>`));
  ...
  get selectElement() {
    return this.querySlotted('select');
  }
  ...
  get sheet() {
    return document.body.querySelector(`mdw-sheet#${this.sheetId}`);
  }

  sheetHTML(injectContent) {
    this.sheetId = MDWUtils.uid();
    return `
      <mdw-sheet mdw-modal id=${this.sheetId}>
        <mdw-sheet-content>
          ${injectContent}
        </mdw-sheet-content>
      </mdw-sheet>
